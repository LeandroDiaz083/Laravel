        ...
        <div class="mb-3">
            ...
            <select name="id_metodo_pago" class="form-control" required>
                @foreach($metodos as $metodo)
                    <option value="{{ $metodo->id_metod_pago }}" {{ $pago->id_metodo_pago == $metodo->id_metod_pago ? 'selected' : '' }}>
                        {{ $metodo->formas_pago }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="mb-3">
            ...
            <select name="id_contratacion" class="form-control" required>
                @foreach($contrataciones as $contrato)
                    <option value="{{ $contrato->id_contratacion }}" {{ $pago->id_contratacion == $contrato->id_contratacion ? 'selected' : '' }}>
